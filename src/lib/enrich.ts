export type EnrichedProfile = {
  ...
  locationTier: "small" | "medium" | "large" | "unknown";
  capitalLevel: "very_low" | "low" | "medium" | "high";
  skillTags: string[]; passionTags: string[];
};
const SKILL_CANON: Record<string,string> = { marketing:"marketing", ads:"ads", sales:"sales", seo:"seo", video:"video", editing:"video", operations:"operations", ops:"operations", coding:"dev", developer:"dev", design:"design", copy:"copy", writing:"copy", teach:"teaching", teaching:"teaching", language:"language" };
const PASSION_CANON: Record<string,string> = { cars:"cars", auto:"cars", fitness:"fitness", gym:"fitness", tech:"tech", gadgets:"tech", media:"media", fashion:"fashion", beauty:"beauty", education:"education", kids:"kids", food:"food", pets:"pets" };
const norm = (s:string)=> (s||"").toLowerCase().trim();

function parseLocation(raw:string){
  const t = norm(raw);
  if(!t) return { city:undefined, region:undefined, country:undefined, tier:"unknown" as const };
  ...
  const base = city || t;
  const tier = LARGE.includes(base||"") ? "large" : MED.includes(base||"") ? "medium" : "small";
  return { city, region, country, tier: base ? (tier as "small"|"medium"|"large") : "unknown" };
}
function capitalToLevel(n:number){ if (!Number.isFinite(n) || n<=0) return "very_low"; if(n<300) return "low"; if(n<2000) return "medium"; return "high"; }
